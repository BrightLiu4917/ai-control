...
3. **自动检查**（内置在 check/ship 命令里的程序，共两道机械门禁）：变更单声明动表却想走快速通道？拦。声明改接口却没写异常测试用例？拦。说测试通过但测试报告里没有对应记录或有失败？拦。（数据库确认属于第 2 层——靠契约约束 AI 停下等你，ship 时会提示做独立审查，不是机械拦截。）
...
├── openspec/        变更单记录，每个需求一个文件夹（简化格式，不兼容 OpenSpec CLI）
...
零运行时依赖（只需 Node ≥18.17 + git）；9 个端到端验收测试 + 6 个单元测试 + 规则库引用自检（防悬空引用）；代码量硬预算写进 CI（内核 ≤1500 行、契约 ≤100 行、文档 1 份），超支即红——防止工具本身变臃肿。
